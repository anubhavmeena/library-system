...
struct AdminCreateMembershipView: View {
    ...
    @State private var pendingAmountStr = ""
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        ...
                        PrimaryButton("Create Membership", isLoading: vm.isLoading) {
                            ...
                            let paid    = Double(paidAmountStr)
                            let pending = Double(pendingAmountStr)
                            vm.createCashMembership(
                                studentId: studentId, planId: plan.id,
    ...
    private var paymentSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Payment (Cash)").font(.labelMedium).foregroundColor(.textSub)
            HStack(spacing: 12) {
                AppTextField(label: "Paid Amount", text: $paidAmountStr,
                             placeholder: "e.g. 800", leadingIcon: "indianrupeesign")
                AppTextField(label: "Pending Amount", text: $pendingAmountStr,
                             placeholder: "e.g. 200", leadingIcon: "clock")
            }
            Text("Leave blank to use plan price. Pending = balance not yet collected.")
