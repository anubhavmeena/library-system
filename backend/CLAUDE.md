...
`ExpiryReminderScheduler` runs two independent daily jobs. `sendExpiryReminders()` runs at `0 0 9 * * *` (JVM-default timezone) and only fires Kafka events at the **7-day and 3-day** thresholds (`daysLeft == 7 || daysLeft == 3`); after firing it sets `Membership.reminderSent = true` so the record is excluded from future queries. `markExpiredAndStartGrace()` runs at `0 0 5 * * *` in `Asia/Kolkata` and transitions `ACTIVE` memberships whose `endDate` has passed into `GRACE` (or `EXPIRED`, if a `QUEUED` renewal takes over) — also triggerable on demand via `POST /api/admin/memberships/run-expiry-check`. The service is deployed at **replicas: 1** to prevent duplicate scheduler firings — if you scale it, add ShedLock.
